...
<p>&emsp;The 8-puzzle is a well-known game that consists of a board, eight tiles, and an empty tile space. 
...
determine which one is the most effective.</p>
...
<p>&emsp;A data structure was created to fulfill these requirements. This structure, built in a Python class, 
...
<p>&emsp;If the board is always complete, the AI has nothing to solve. Rectifying this requires the board to 
...
retrace her steps. The program can accomplish this at this point.</p> 

<p>&emsp;The board is now randomly mixed up and ready to be solved by AI. The algorithm developed to 
...
there are likely many solutions located on the tree.</p>
...
<p>&emsp;If A* searches through the entire search tree, then the number of nodes expanded, and the time 
...
solution without searching through the entire tree.</p>

<p>&emsp;Each node on the search tree is given a value. The unexplored move with the lowest value is 
...
in difficult problems (Phillips, 2022a).</p>

<p>&emsp;For this problem, four heuristics will be considered. The first heuristic only considers the cost so 
...
<p>&emsp;After writing the algorithm and the four heuristics, they must be analyzed. Each heuristic is 100 
...
number of nodes at each level of the tree (branching factor).</p>

<p>&emsp;It is at this point that pseudo-random moves must be revisited. Recall that the user enters a seed 
...
comparison between heuristics.</p>
...
<p>&emsp;As the heuristics increase in value, admissibility becomes a concern. A heuristic is inadmissible if 
...
is no evidence to suggest that the heuristics are not admissible.</p>

<p>&emsp;If each heuristic finds the best solution to a given board, then the separation between them is 
...
making them better than the ones before.</p>

<p>&emsp;In conclusion, the descriptive statistics suggest that each proposed heuristic is admissible. That 
...
fully compare the four heuristics and confirm the findings presented here.</p>
